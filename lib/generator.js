var path = require('path'),
    _ = require("lodash");
var logger = require('./util').log;//require('loggy');
var methods = [
...
module.exports = function createRoutes(router, files, dir){
  ...
  var shortList = _.uniq(_.map(files, function (file) {
    return file.replace(path.extname(file), '');
  })).sort();

    _.each(shortList, function (file) {
        var segs = file.split('/')
            , seqNumber;

        var fileName = segs.pop(),
            //ext = path.extname(fileName),
            nameSegs = path.basename(fileName).split('.'),
            params = {
